...
# ====================================================================
# PDF GENERATION FUNCTION (2 หน้า: หน้าแรกตาราง หน้าสองกราฟ)
# ====================================================================
def generate_pdf_report(df_history, age, gender, weight, height):
    pdf = FPDF()
    
    # หน้าที่ 1: ตารางสรุปผลค่าวิกฤตสูงสุด
    pdf.add_page()
    ...
    if not df_history.empty:
        ...
        # หน้าที่ 2: กราฟเส้น Kinematics
        pdf.add_page()
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "Knee Kinematics & Injury Risk Visualization", ln=True, align="L")
        pdf.ln(5)

        plt.figure(figsize=(7, 4))
        plt.plot(df_history["Left Knee"], label="Left Knee Angle", color="blue", linewidth=1.5)
        plt.plot(df_history["Right Knee"], label="Right Knee Angle", color="green", linewidth=1.5)
        plt.plot(df_history["Risk"], label="ACL Risk (%)", color="red", linestyle="--", linewidth=1.5)
        plt.title("Knee Kinematics & Injury Risk Over Time", fontsize=12)
        plt.xlabel("Frames", fontsize=10)
        plt.ylabel("Degrees / Percentage", fontsize=10)
        plt.grid(True, linestyle=":", alpha=0.6)
        plt.legend(loc="upper left")
        plt.tight_layout()
        
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_img:
            plt.savefig(tmp_img.name, dpi=200)
            plt.close()
            pdf.image(tmp_img.name, x=10, y=30, w=190)
...
if save_data:
    ...
    if st.session_state.history_data:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"ACL_Report_2Pages_{int(time.time())}.pdf",
...
if run:
    ...
    if cap is not None:
        ...
        # กำหนดค่าคงที่ Delta Time อิงตามความเร็วเฟรมจริงของกล้อง ป้องกันค่าความเร่งค้างเป็นศูนย์
        dt = 1.0 / fps
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                # คำนวณความเร็วและความเร่งแบบ Finite Difference มั่นคงสูง
                ...
                # เรียกคำนวณคะแนนดิบจากไฟล์กลไกหลัก
                raw_risk = acl_risk_score(left_angle, right_angle, asymmetry, left_valgus, right_valgus, velocity, acceleration, age, gender)
                
                # [จุดแก้ไขอาการค้าง 25%]: หากมุมเข่ายังเหยียดตรง (ยืนเฉย ๆ ไม่เคลื่อนไหว) บังคับให้ความเสี่ยงทางกลศาสตร์เป็น 0%
